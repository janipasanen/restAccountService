...
class PostResourceHandler @Inject()(
    routerProvider: Provider[PostRouter2],
    postRepository: PostRepository2)(implicit ec: ExecutionContext) {

  def create(postInput: PostFormInput)(implicit mc: MarkerContext): Future[PostResource] = {
    val data = PostData2(PostId2("999"), postInput.title, postInput.body)
  ...
  def lookup(id: String)(implicit mc: MarkerContext): Future[Option[PostResource]] = {
    val postFuture = postRepository.get(PostId2(id))
